...
if %_ena%==3 (
set _status=Activado
set _col=%_Green%
)

if %_dis%==3 (
set _status=Desactivado
set _col=%_Red%
)
...
echo.
call :_color2 %_White% "        " %_Green% "  Create By Joseph NC"
echo:          _____________________________________________        
echo:          +++++++++++++++++++++++++++++++++++++++++++++  
echo:          _____________________________________________   
echo:                                                          
echo:          [1] Activar IDM PERMANENTE                              
echo:          [2] Restablecer activacion y Tiempo de Prueba 
:: echo:          _____________________________________________   
:: echo:                                                          
:: call :_color2 %_White% "          [3] Alternar cortafuegos de Windows  " %_col% "[%_status%]"
echo:          _____________________________________________   
echo:                                                                                               
echo:          [4] Exit                                        
echo:       ___________________________________________________
echo:   
call :_color2 %_White% "        " %_Green% "  Enter a menu option in the Keyboard [1,2,4]"
choice /C:1234 /N
set _erl=%errorlevel%
...
call :_color %Green% "IDM Activation Cleaned - Trial is successfully reset in the registry."
) else (
...
if not exist "!IDMan!" (
call :_color %Red% "IDM [Administrador de descargas de Internet] no está instalado".
echo Puede descargarlo desde https://www.internetdownloadmanager.com/download.html
goto done
)
...
echo Bloqueando claves de registro...
echo:
call :action
...
echo:
echo Aplicando detalles de registro...
@echo off
set "name=Joseph NC"
set "email=Joseph@nc.com"
set "serial=FOX6H-3KWH4-7TSIN-Q4US7"

reg add "HKCU\SOFTWARE\DownloadManager" /v FName /t REG_SZ /d "%name%" /f >nul
reg add "HKCU\SOFTWARE\DownloadManager" /v LName /t REG_SZ /d "" /f >nul
reg add "HKCU\SOFTWARE\DownloadManager" /v Email /t REG_SZ /d "%email%" /f >nul
reg add "HKCU\SOFTWARE\DownloadManager" /v Serial /t REG_SZ /d "%serial%" /f >nul

echo:
echo Activando algunas descargas para crear ciertas claves de registro, espere...
...
call :_color %Magenta% "Vuelva a intentarlo; deshabilite el firewall de Windows con las opciones de script."
)
...
set DownloadManager=HKCU\Software\DownloadManager
...
"LastCheckQU"
    ...
    reg delete "%DownloadManager%" /v %%K /f >nul
)
...
echo:
echo Agregando clave de registro...
echo:
